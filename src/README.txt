...
- X Install and use the zipcodes package (https://www.npmjs.com/package/zipcodes) to display the city and state of the teacher from their zipcode (City, State).
- X Display the subjects on the teacher card by most experience first.
- X Implement the text search. this should be able to make a match with any attribute of the teacher, eg. name, subjects, etc.
- X Implement the Sort By Dropdown. For the Years experience dropdown, use the subject they have the most experience in.
  This will require you to look at the React Select documentation (https://react-select.com/home).
- X Implement the existing grade levels filters.
- X Add a states filter section in the Filter Bar that allows filtering candidates by their state (as determined by zipcode). 
